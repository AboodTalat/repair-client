// ISO 3166-1 country list with dialing codes. Used by the contact form's
// country-code picker (`CountryCodePicker.jsx`). Flag emoji is derived at
// render time from `iso2` via `flagEmoji()` so the table stays compact.
...
// Symbol (U+1F1E6 + offset). Returns "" for invalid input so callers can
// safely concatenate.
export function flagEmoji(iso2) {
  if (typeof iso2 !== "string" || iso2.length !== 2) return "";
  const A = 0x1f1e6;
  const a = "A".charCodeAt(0);
  const cp = (i) => A + (iso2.toUpperCase().charCodeAt(i) - a);
